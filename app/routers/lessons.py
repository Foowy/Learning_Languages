...
async def complete_lesson(
    ...
    user_id = 1
    await db.execute(
        "INSERT OR REPLACE INTO lessons (lesson_id, unit, language, user_id, completed_at, quiz_score) VALUES (?,?,?,?,?,?)",
        (lesson, unit, "japanese", user_id, datetime.utcnow().isoformat(), body.quiz_score)
    )
    cursor = await db.execute("SELECT id FROM cards WHERE unit=? AND lesson=?", (unit, lesson))
    for row in await cursor.fetchall():
        await db.execute(
            "INSERT OR IGNORE INTO progress (card_id, user_id, due_date, interval_days, ease_factor, review_count)"
